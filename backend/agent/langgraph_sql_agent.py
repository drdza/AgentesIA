...
async def enhance_question_node(state: AgentState):
    try:
        ...
        enhancer_question, duration, _ = await asyncio.to_thread(call_model, "mistral", enhancer_prompt, state['question'])
        #logger.info(enhancer_question)
...
async def generate_flow_or_context_node(state: AgentState):
    ...
    try:
        flow_prompt = await asyncio.to_thread(load_prompt_template, state['domain'], "flow_generator_rag.txt")
        flow_text, duration, _ = await asyncio.to_thread(call_model, "mistral", flow_prompt, state['enhanced_question'])
...
async def generate_sql_node(state: AgentState):
    try:
        sql_prompt_template = await asyncio.to_thread(load_prompt_template,state['domain'], "system_context_rag.txt")
        prompt = sql_prompt_template.format(
            flow=state.get("flow", ""),
            context=state.get("rag_context", "")
        )
        raw_sql, duration, _ = await asyncio.to_thread(call_model, "mistral", prompt, state['enhanced_question'])
